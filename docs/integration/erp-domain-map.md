...
| Shipment | **DOES NOT EXIST** | — | — | — | No dedicated Shipment/tracking/courier-integration model was found anywhere in the 79-model schema or in the modules inspected. Fulfillment/delivery is tracked entirely through `SalesOrder.primaryStatus` (`out_for_delivery`, `delivered`) plus `StockMove` — terminology in the code ("خروج مع المندوب" = "goes out with the delivery rep") confirms an own-delivery-rep model, not a third-party courier API integration. **Phase 8.5: confirmed, not just inferred** — a full case-insensitive source grep for `courier`/`3pl`/`aramex`/`bosta`/`fedex`/`dhl` across `src/` returned zero real matches (the only hits were the `"Courier New"` font family in print templates). This is now a proven negative, not a strong inference. | — |
