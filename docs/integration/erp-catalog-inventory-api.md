...
**⚠️ CORRECTED — Inventory Integration milestone.** Everything below this notice through §18 ("Phase 9.5R — inventory identity correction") describes an ERP-side implementation (`skus` wire option, `getSellableAvailabilityBySkus()`/`getSellableAvailabilityByVariantIds()`, `findVariantIdsExisting()`, and the specific test counts cited at the end of this document) that a direct audit of the real ERP codebase (this milestone) confirmed **does not exist and never did**. This document was written describing planned/assumed ERP-side work at a time when the Website's own adapter code was already built but had never actually been run against, or verified against, a real ERP instance.

**The real, current, verified contract** (confirmed both by reading `src/app/api/v1/integrations/website/inventory/availability/route.ts` directly and by live calls against deployed ERP staging this milestone): body is `{ "variantIds": string[] }` **only** — there is no `skus` option, and never was, on this specific endpoint. 1 to 200 entries (`MAX_IDS_PER_REQUEST`, ERP-side), matching the Website adapter's own batching limit exactly. Implemented by `getAvailabilityForWebsite()` (`website-inventory.service.ts`), which resolves bundle/stock-source-redirect components via `resolveOrderLineComponents()` (`product-variant.service.ts`) and computes the final sellable quantity via `computeSellableAvailability()` (`website-inventory-availability.ts`) — real function names, in real files, as of this milestone. The rest of this section's original text is preserved below only for historical context; treat any claim about a `skus` request shape or the named functions above as inaccurate.
