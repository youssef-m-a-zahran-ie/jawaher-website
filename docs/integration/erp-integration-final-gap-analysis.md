...
| Shipping/courier relationship to ERP | Undecided in Phase 5 | **No Shipment/courier entity anywhere in the schema or source code — Phase 8.5 confirmed via full source grep, not just a schema inference.** Own-delivery-rep model, not a 3PL API integration | Resolved: Website's own `ShippingProvider` stays independent — no competing courier-tracking source exists to reconcile against | LOW | **H — resolved** (technical fact confirmed; whether the business ever *wants* a real courier later is a separate, unrelated future decision) | — |
...
| RLS `FORCE ROW LEVEL SECURITY` status | N/A — pre-existing ERP platform risk, unrelated to this integration | **RESOLVED — Phase 8.5 verified live**: `FORCE ROW LEVEL SECURITY` is set on all 88/88 public-schema tables; `anon`/`authenticated` are genuinely subject to it (`rolbypassrls: false`); the app's own connection deliberately bypasses it (`rolbypassrls: true`), exactly as ADR-0001 describes. **Precisely: RLS is not what isolates the Website integration path's tenant data — the application-level `TenantContext` mechanism does, verified separately by a cross-tenant test.** No defect found. See `erp-pre-integration-closure.md` §5. | None remaining | — | **H — resolved** | — |
