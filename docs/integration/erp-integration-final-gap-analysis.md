...
| RLS `FORCE ROW LEVEL SECURITY` status | N/A — pre-existing ERP platform risk, unrelated to this integration | **RESOLVED — Phase 8.5 verified live**: `FORCE ROW LEVEL SECURITY` is set on all 88/88 public-schema tables; `anon`/`authenticated` are genuinely subject to it (`rolbypassrls: false`); the app's own connection deliberately bypasses it (`rolbypassrls: true`), exactly as ADR-0001 describes. No defect found. See `erp-pre-integration-closure.md` §5. | None remaining | — | **H — resolved** | — |
