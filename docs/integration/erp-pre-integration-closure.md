...
**Zero new BLOCKER-severity items found.** This phase closed the single largest pre-existing open item from Phase 6/7 — `FORCE ROW LEVEL SECURITY`'s configuration status — now confirmed live, correctly enabled on 88/88 tables. Precisely stated (see §5 for the full, corrected terminology): RLS is genuinely enforced for the `anon`/`authenticated` access paths; the application's own database connection deliberately bypasses it (`rolbypassrls=true`, by design, per ADR-0001); the Website integration path's actual tenant isolation is provided by the application-level `TenantContext` mechanism, not by RLS, and that mechanism was directly tested (a new cross-tenant test) rather than assumed. This phase also ran a genuine end-to-end connectivity test against a real running ERP with real credentials (not mocked), and confirmed — by full source grep, not inference — that no courier/3PL integration exists anywhere in the ERP. One ERP code change was made (a new test), tied directly to the tenant-isolation verification this phase performed; no other code changes were necessary. All remaining open items are genuine business decisions (unchanged in substance from Phase 7, consolidated here) or items that structurally cannot be resolved before the Catalog/Order integration phases exist.

> **Phase 8.5R addendum (2026-09-11):** the development API key used during this phase's live verification was exposed in Claude Code's execution/session output. It has been revoked and rotated — see §15. It was never committed to either repository (verified against full git history, both repos, all refs) — the exposure was confined to session/tool output, not source control. This document's RLS wording was also corrected for precision at the same time (§5) — the underlying architecture and findings are unchanged; only the terminology was tightened.
...
**Precise terminology (Phase 8.5R correction)** — four distinct facts, not one blended claim:

**(a) RLS / FORCE RLS database configuration.** `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` and `... FORCE ROW LEVEL SECURITY` are both genuinely applied, live, on all 88/88 tables in the `public` schema (`relkind='r'`): `count(*) filter (where relforcerowsecurity) = 88`, i.e. 100%; a supplementary query for tables *without* `relforcerowsecurity` returned an empty set. A targeted sample (`companies`, `users`, `sales_orders`, `sales_order_lines`, `business_partners`, `stock_quants`, `stock_reservations`, `stock_moves`, `integration_secrets`, `company_integration_connections`, `integration_connectors`) was individually confirmed `relrowsecurity=true, relforcerowsecurity=true`. This is a database-configuration fact only — it says what Postgres is instructed to enforce, not who it actually restricts (see (c)).

**(b) Application-level `TenantContext` isolation — the mechanism that actually protects the Website integration path.** The new `/api/v1/integrations/website/health` route resolves its tenant via `getSystemTenantContext(connection.companyId)` — no Postgres role switch, no session variable, no RLS-relevant identity change of any kind. Every downstream repository call still hand-writes `where: { companyId: ctx.companyId, ... }` (unchanged since Phase 6/7, re-confirmed by `npm run check:tenant-scope` passing with zero violations both this phase and Phase 8). **This — not RLS — is what prevents the integration path from reading or writing another company's data**, exactly as ADR-0001 already established for every other system-triggered path (webhooks, jobs) in this codebase.

**(c) The application database role has `rolbypassrls = true`, by design.** `select rolname, rolsuper, rolbypassrls, rolcanlogin from pg_roles` shows `postgres` (the app's `DATABASE_URL` role) and `service_role` both have `rolbypassrls: true, rolsuper: false` — **not** literal Postgres superusers (correcting ADR-0001's own informal "superuser" phrasing to the precise mechanism), but a role attribute that makes RLS's row filters **not apply to this role's own queries, including (a)'s FORCE setting** — `FORCE ROW LEVEL SECURITY` overrides only the *table-owner* exemption, not the `BYPASSRLS` role attribute (a distinct, deliberate Postgres mechanism). **RLS therefore provides zero defense-in-depth for the application's own connection — this is expected and unchanged from ADR-0001's original design, not a new finding, and (a)'s 88/88-tables result must never be read as "therefore the app role is restricted too."** `anon` and `authenticated` (the roles PostgREST/Supabase client libraries use, a *different* access path than the application's own Prisma traffic) both show `rolbypassrls: false` — for those roles specifically, and only those, RLS's row filters genuinely apply, which is the real, live defense-in-depth ADR-0001 claims for that separate path.

**(d) Cross-tenant confusion in the new integration-auth code was directly tested, not inferred from (a)/(c) above.** See §4.1: a new test (`service.test.ts`, "never resolves Company A's connection using Company B's real api_key") proves Company B's genuinely-correct, correctly-encrypted key is rejected against Company A's connection id. This test exercises (b) — the application-level resolution logic — directly; it does not exercise or depend on RLS in any way (the test uses an in-memory fake `db`, never a real Postgres connection).

**Conclusion, stated precisely**: the ERP's tenant isolation for this integration path is provided entirely by (b), the application-level `TenantContext` mechanism, verified in (d). RLS (a) is correctly and fully configured, and does provide genuine defense-in-depth for the `anon`/`authenticated` access paths — but, by design (c), it provides none for the application's own connection, and this document must not be read as claiming otherwise. This was previously tracked as "unconfirmed"; it is now **confirmed, and matches the architecture ADR-0001 always described** — not a newly-discovered gap, and not a claim that RLS itself stops the application role.
...
**VERIFIED** — precisely: RLS/FORCE RLS configuration verified correct (88/88 tables) and genuinely enforced for `anon`/`authenticated`; the application's own connection verified to bypass RLS by design (`rolbypassrls=true`, ADR-0001, not a defect); actual tenant isolation for the Website integration path verified to come from the application-level `TenantContext` mechanism, confirmed by a direct cross-tenant test (§4.1/§5(d)) — not from RLS. See §5 for the full, corrected statement.
...
## 15. Credential rotation (Phase 8.5R)

The development `website` connector API key used during this phase's own live verification (§6) was exposed in Claude Code's execution/session output while running `curl`/test commands. Treated as compromised even though development-only, per instruction.

**Audit — was it ever committed?** No. Searched both repositories' working trees and **full git history, all commits, all refs** (`git log --all -p | grep`) for the exact exposed key string: zero matches in either repository, in the working tree or in any commit. The exposure was confined to this session's tool-call/console output (curl responses, a temporary uncommitted test file's process — the file itself read the key from a shell environment variable, never hardcoded it, and was deleted before any commit). **No git history rewrite was needed or performed**, since there was nothing in history to remove.

**Rotation performed**: `npm run provision:website-integration` was re-run for the same connection (`CompanyIntegrationConnection` id unchanged — it is a public identifier, not a secret, so rotating it carries no security benefit). `IntegrationSecretService.storeSecret()`'s existing rotate-only design (Phase 6 finding, unchanged) atomically revoked the old `IntegrationSecret` row and created a new active one — confirmed at the database level (no plaintext read): exactly one `revoked` row (`revokedAt` populated, timestamped) and one `active` row (`revokedAt: null`) exist for this connection's `api_key` secret type.

**Verification performed, live, against the real running ERP**:
- Old key → `401` (three ways: direct `curl`, and via the real Website `erpIntegrationService.checkConnection()` adapter code — not mocked).
- New key → `200` (same three ways).
- No secret value was written into any committed file, this document, or the final handoff report. The new key exists only in this local development environment's database (encrypted) and wherever the operator running this phase chooses to store it for local `.env` use — **not printed here**.

**No related cleanup was performed beyond the rotation itself** — no unrelated files were touched, per the brief's explicit instruction not to introduce unrelated cleanup.
