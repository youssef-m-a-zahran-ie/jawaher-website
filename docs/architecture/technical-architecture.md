...
**No full CMS is justified at this stage** — every content type above is either simple structured data or reuses an existing template; introducing a headless CMS product would add an external dependency and an extra content-sync problem without a corresponding need (the business already has a place to supply raw content: `_reference/`). Revisit only if the content team's editing workflow genuinely outgrows direct database-backed editing via an internal/admin surface (not designed in this stage).
